...
#----------------------------------------------------------------------------------------------
    ...
    # Wyznaczanie rownan prostych ukladu wspolrzednych stolu wzgledem ukladu bazy
    Xx = math.cos(alpha)*1.1 + T_B_Table.p.x()
    Xy = math.sin(alpha)*1.1 + T_B_Table.p.y()
    Yx = math.cos(alpha)*0.9 + T_B_Table.p.x()
    Yy = math.sin(alpha)*0.9 + T_B_Table.p.y()
    # Posta OX (y = aox*x + box)
    aox = (T_B_Table.p.y() - Xy)/(T_B_Table.p.x() - Xx)
    box = Xy - Xx*aox
    # Posta OY (y = aox*x + box)
    aoy = (T_B_Table.p.y() - Yy)/(T_B_Table.p.x() - Yx)
    boy = Yy - Yx*aoy
    # Wyznaczanie odleglosci prostych od poczatku ukladu wspolrzednych bazy
    dx = math.fabs(box)/math.sqrt(math.pow(aox, 2) + 1)
    dy = math.fabs(boy)/math.sqrt(math.pow(aoy, 2) + 1)

    # Wyznaczanie wspolrzednych wierzcholkow stolu
    Ax = math.cos(alpha)*(-1.1) - math.sin(alpha)*(-0.9) + T_B_Table.p.x()
    Ay = math.sin(alpha)*(-1.1) + math.cos(alpha)*(-0.9) + T_B_Table.p.y()

    Bx = math.cos(alpha)*(1.1) - math.sin(alpha)*(-0.9) + T_B_Table.p.x()
    By = math.sin(alpha)*(1.1) + math.cos(alpha)*(-0.9) + T_B_Table.p.y()

    Cx = math.cos(alpha)*(1.1) - math.sin(alpha)*(0.9) + T_B_Table.p.x()
    Cy = math.sin(alpha)*(1.1) + math.cos(alpha)*(0.9) + T_B_Table.p.y()

    Dx = math.cos(alpha)*(-1.1) - math.sin(alpha)*(0.9) + T_B_Table.p.x()
    Dy = math.sin(alpha)*(-1.1) + math.cos(alpha)*(0.9) + T_B_Table.p.y()

    # Odleglosc wierzcholkow od bazy
    dA = math.sqrt(math.pow(Ax, 2) + math.pow(Ay, 2))
    dB = math.sqrt(math.pow(Bx, 2) + math.pow(By, 2))
    dC = math.sqrt(math.pow(Cx, 2) + math.pow(Cy, 2))
    dD = math.sqrt(math.pow(Dx, 2) + math.pow(Dy, 2))

    if dx >= 0.9 and dy >= 1.1:  # Najblizej robota jest naroznik (tylko ktory??)
        if dD > dA and dB > dA:   # Naroznik A
            posX = Ax
            poxY = Ay
        elif dA > dB and dC > dB: # Naroznik B
            posX = Bx
            poxY = By
        elif dB > dC and dD > dC: # Naroznik C
            posX = Cx
            posx = Cy
        else:                     # Naroznik D
            posX = Dx
            posx = Dy           
    elif dx < 0.9:  # Najblizej robota jest punkt na krawedzi rownoleglej do osi OX (tylko na ktorej??)
       if dB < dC:   # Krawedz AB
       
       else:         # Krawedz CD

    else:           # Najblizej robota jest punkt na krawedzi rownoleglej do osi OY (tylko na ktorej???)
       if dA < dB:   # Krawedz AD
    ...
    # cwiartka 1
    if T_B_Table.p.x() > 0 and T_B_Table.p.y() >= 0:
        angle = math.atan(T_B_Table.p.y()/T_B_Table.p.x())
    # cwiartka 2
    elif T_B_Table.p.x() <= 0 and T_B_Table.p.y() >= 0:
        angle = 1.56
    # cwiartka 3
    elif T_B_Table.p.x() <= 0 and T_B_Table.p.y() < 0:
        angle = -1.56
